...
def get_data(doc):
    site_wrapper = doc.find(class_="f1-menu-wrapper flex flex-col gap-micro f1-filters-wrapper max-h-[7.5em] max-laptop:bg-brand-offWhite overflow-y-auto p-normal relative")
    main = site_wrapper.find(class_="bg-brand-white f1-utils-inner-padding")
    inner_class = main.find(class_="f1-table f1-table-with-data w-full")
...
def get_input():
    ...
    if len(ipt) > 1:
        while int(ipt[0]) < 1950 or int(ipt[1]) > 2022:
            ipt = input("No such year. Dates should be given between 1950 and 2022.\n")
            ipt = ipt.split(",")
...
def save_dict(path, dict, flag):
    ...
    with open(path, "a", encoding="utf-8") as output:
        txt = (
            "-" * 24
            + " Total points collected by drivers/teams during this period"
            + "-" * 30
            + "\n"
        )
        output.write(txt)
...
def main():
    ...
    for y in year:
        ...
        url = "https://www.formula1.com/en/results/" + str(y) + "drivers"
        doc = get_page(url)
        tbody = get_data(doc)
